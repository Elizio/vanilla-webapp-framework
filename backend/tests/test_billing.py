...
def _auth_token(client):
    ...
    client.post('/api/register', json={'username': 'payer', 'password': 'secret'})
    resp = client.post('/api/login', json={'username': 'payer', 'password': 'secret'})
...
def test_webhook_order_grants_entitlement(test_app, test_client, test_db):
    ...
    badge = test_client.get(
        '/api/supporter-badge', headers={'Authorization': f'Bearer {token}'}
    )
